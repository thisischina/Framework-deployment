...
//}
var ispost = false, _seccode = '';
var is_ie = (function(){
    var v = 3, div = document.createElement('div'), all = div.getElementsByTagName('i');
    while (div.innerHTML = '<!--[if gt IE ' + (++v) + ']><i></i><![endif]-->',all[0]);
    return v > 4 ? v : false ;
}());
var head_conf = { head: "Core" };
//只包含加载
...
(function(win) {
  JSS = {
		...
		json      : "json2",
		ajaxform  : "plus/ajaxform",
		lazyload  : "plus/lazyload",
		upload    : "plus/uploadify",
		upload5   : "plus/htmlupload",
		imgarea   : "plus/imgareaselect.pack",
		inscroll  : "plus/infinitescroll",
		fadeslides: "plus/responsiveslides",
		carousel : "plus/carousel",
		imageZoom : "plus/imageZoom",
		gallery   : "gallery",
		share     : "share",
		waterfall : "plus/waterfall",
		UI        : "plus/jquery-ui-custom",
		mousewheel: "plus/mousewheel",
		jscrollpane: "plus/jscrollpane",
		montage   : "plus/montage",
		placeholder: "plus/placeholder" //ie
  }
  for(var i in JSS) {
	  if (JSS.hasOwnProperty(i)) {
		  JSS[i] = CONFIG.jsPath + JSS[i] +'.js?v=' + CONFIG.version;
	  }
  }
  win.Core = win.Core || {};
  Core.use = function() {
	  ...
	  for( var i = 0;i < len;i++ ) {
		  if(typeof args[i] === 'string' && JSS[args[i]]) {
			  args[i] = JSS[args[i]];
		  }
	  }
	  Core.js.apply(null,args);
  };
  Core.com = Core.user = {};
})(window);